...
router.get("/:contest_id",
    async (req, res) => {
        try {
            ...
            const data = await models.private_contests.findByPk(contestId, {
                attributes: ['id', 'total_teams', 'entry_fee', 'max_team', 'prize', 'is_confirmed', 'prize_breakup',
                    [db.sequelize.literal(`(SELECT COUNT(*) FROM user_private_contests WHERE private_contest_id = private_contests.id)`), "total_joined"],
                    [db.sequelize.literal(`(SELECT IF(COUNT(*) = 0, false, true) FROM user_private_contests WHERE user_id = "${req.userId}" AND private_contest_id = private_contests.id)`), "has_joined"]
                ],
            });
