        ...
        try {
            ...
            const contests = await adminModels.contests.findAll({
                where: {
                    ...
                    id: {
                        [Op.in]: adminDb.sequelize.literal(`(SELECT contest_id FROM user_contests as uc WHERE uc.user_id = "${req.userId}" AND uc.contest_id = contests.id)`)
                    },
                },
                include: [
                    {
                        model: adminModels.user_contests,
                        as: 'user_contests',
                        attributes: ['id'],
                        include: [
                            {
                                model: adminModels.user_teams,
                                as: 'user_team',
                                attributes: ['name']
                            }
                        ]
                ...
                attributes: [
                    ...
                    [adminDb.sequelize.literal(`(SELECT SUM(prize) FROM user_contests WHERE contest_id = contests.id) AND user_contests.user_id = "${req.userId}"`), "winning_amount"],
                ],
            });
