...
// Book A Free Demo Drop-down JS 
$(document).ready(function () {
	$(document).on('click', '.book-btn', function(){ 
		$('.book-demo-dropdown').toggle();
		$('.book-btn').toggleClass('show');
	});
	$('*').click(function(e){
		if (e.target != this)return;
		if(!$(this).closest('.book-btn').length && !$(this).closest('.book-demo-dropdown').length){
			$('.book-demo-dropdown').hide();
			$('.book-btn').removeClass('show');
		}	
	});
});